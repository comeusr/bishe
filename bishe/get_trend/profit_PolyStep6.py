...
path=r'C:\Users\Ziyi Wang\Desktop\bishe\realize wangyi\bishe\results\profit.csv'
...
def getPoly2MSE(i,j,a,b,c):
    ...
    y2 = np.array(list(Poly2func(x,a,b,c)))
    error = y2-y
    temp = np.power(error,2)
    n = np.sum(temp)
    return n/len(temp)
...
if __name__ == '__main__':
    ...
    dataframe_params.to_csv(resultsPath+'Profit-'+'Poly''-'+str(step)+'-Params'+'-'+str(start)[0:10]+".csv")
    ...
    dataframe_Evaluation.to_csv(resultsPath+'Profit-'+'Poly'+'-'+str(step)+'-Evaluation'+'-'+str(start)[0:10]+".csv")
    ...
    dataframe_Residual.to_csv(resultsPath+'Profit-'+'Poly'+'-'+str(step)+'-Residual'+'-'+str(start)[0:10]+".csv")
