...
path=r'C:\Users\Ziyi Wang\Desktop\bishe\realize wangyi\Data\change_point.xlsx'
...
def getPoly2MSE(i,j,a,b,c):
    #a,b,c分别是二次项、一次项和常参数
    # x = np.array(range(1, j-i+2))
    x = np.array(df.loc[i:j,0])
    y = np.array(df.loc[i:j,1])
    y2 = Poly2func(x,a,b,c)
    temp = np.power(y2-y,2)
    n = np.sum(temp)
    return n/len(temp)

def getPoly3MSE(i,j,a,b,c,d):
    # a,b,c,d分别是三次项、二次项、一次项和常参数
    # x = np.array(range(1, j - i + 2))
    x = np.array(df.loc[i:j,0])
    y = np.array(df.loc[i:j,1])
    y2 = Poly3func(x,a,b,c,d)
    temp = np.power(y2-y,2)
    n = np.sum(temp)
    return n/len(temp)

def getPoly4MSE(i,j,a,b,c,d,e):
    # a,b,c,d,e分别是四次项、三次项、二次项、一次项和常参数
    # x = np.array(range(1, j-i+2))
    x = np.array(df.loc[i:j,0])
    y = np.array(df.loc[i:j,1])
    y2 = Poly4func(x,a,b,c,d,e)
    temp = np.power(y2-y,2)
    n = np.sum(temp)
    return n/len(temp)
...
def predictPoly2(i,j,a,b,c):
    #i,j是数据起始点
    #a,b,c分别是二次函数二次项、一次项和常参数
    # x = np.array(range(1, step + 1))
    x = np.array(df.loc[i:j, 0])
...
# def overFitted(FuncName,testLength,start,end,p4=0,p3=0,p2=0,p1=0,p0=0):
#     #用来防止过拟合，在延长一个testLength来评估函数拟合效果,返回MSE
#     #end是拟合窗口的结束位置
#     if FuncName == 'Poly2':
#         result = getPoly2MSE(start,end+testLength,p2,p1,p0)
#     elif FuncName == 'Poly3':
#         result = getPoly3MSE(start,end+testLength,p3,p2,p1,p0)
#     elif FuncName == 'Poly4':
#         result = getPoly4MSE(start,end+testLength,p4,p3,p2,p1,p0)
#     return result
...
if __name__ == '__main__':
    ...
    for i in range(0, df.shape[0]):
        ...
        # overFittedMSE = {'Poly2':1000,'Poly3':1000,'Poly4':1000}
        ...
        for key in FuncDic:
            try:
                trend = FuncDic[key](i, iEnd)
                print('外循环', i, ',内循环', key, '标签已获取！')
                if key == 'Poly2':
                    MSE[key] = MSEfunc[key](i,iEnd,trend[0],trend[1],trend[2])
                    MAPE[key] = MAPEfunc[key](i,iEnd,trend[0],trend[1],trend[2])
                    MAD[key] = MADfunc[key](i,iEnd,trend[0],trend[1],trend[2])
                    Residual[key] = getResidual(i,i+step-1,predictPoly2(i,i+step-1,trend[0],trend[1],trend[2]))
                    # overFittedMSE[key] = overFitted(key,i,i+step-1,trend[0],trend[1],trend[2])
                    trend = [0,0]+trend
                elif key == 'Poly3':
                    MSE[key] = MSEfunc[key](i,iEnd,trend[0],trend[1],trend[2],trend[3])
                    MAPE[key] = MAPEfunc[key](i,iEnd,trend[0],trend[1],trend[2],trend[3])
                    MAD[key] = MADfunc[key](i,iEnd,trend[0],trend[1],trend[2],trend[3])
                    Residual[key] = getResidual(i,i+step-1,predictPoly3(i,i+step-1,trend[0],trend[1],trend[2],trend[3]))
                    # overFittedMSE[key] = overFitted(key,i,i+step-1,trend[0],trend[1],trend[2],trend[3])
                    trend = [0]+trend
                elif key == 'Poly4':
                    MSE[key] = MSEfunc[key](i,iEnd,trend[0],trend[1],trend[2],trend[3],trend[4])
                    MAPE[key] = MAPEfunc[key](i,iEnd,trend[0],trend[1],trend[2],trend[3],trend[4])
                    MAD[key] = MAPEfunc[key](i,iEnd,trend[0],trend[1],trend[2],trend[3],trend[4])
                    Residual[key] = getResidual(i,i+step-1,predictPoly4(i,i+step-1,trend[0],trend[1],trend[2],trend[3],trend[4]))
                    # overFittedMSE[key] = overFitted(key,i,i+step-1,trend[0],trend[1],trend[2],trend[3],trend[4])
        ...
        data = {
            ...
            'Error':Error,
            'MSE': MSE,
            # 'overFittedMSE':overFittedMSE,
            'MAD': MAD,
            'MAPE': MAPE,
            'residual':Residual,
            # 'overFitted_FinalFunc': min(overFittedMSE,key=overFittedMSE.get),
            # 'overFitted_FinalTrend': Trend[min(overFittedMSE,key=overFittedMSE.get)],
    ...
    for i in range(len(dataList)):
        ...
        # OverFittedFinalFunc.append(dataList[i].get('overFitted_FinalFunc'))
        ...
        # optimalOverFittedMSE.append(dataList[i].get('overFittedMSE')[min(MAPE,key=MAPE.get)])
    ...
    dataframe_params = pd.DataFrame({
        ...
        # 'OverFittedFinalFunc':OverFittedFinalFunc,
        # 'OverFittedMSE':optimalOverFittedMSE
    })
    dataframe_params.to_csv(resultsPath+'Poly''-'+str(step)+'-Params'+'-'+str(start)[0:10]+'(1)'+".csv")
    ...
    dataframe_Evaluation.to_csv(resultsPath+'Poly'+'-'+str(step)+'-Evaluation'+'-'+str(start)[0:10]+'(1)'+".csv")
